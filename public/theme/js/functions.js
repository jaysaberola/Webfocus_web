...
(function($){
	...
	SEMICOLON.header = {
		...
		removeStickyness: function(){
			if( $header.hasClass('sticky-header') ){
				...
				SEMICOLON.slider.slickSliderMenu();
	...
	SEMICOLON.slider = {
		...
		slickSliderMenu: function (onWinLoad) {
			onWinLoad = typeof onWinLoad !== "undefined" ? onWinLoad : false;
			if (
				$body.hasClass("device-xl") ||
				$body.hasClass("device-lg") ||
				...
				let activeSlide = $slider.find(".slick-slide.slick-active .hero-slide");
				SEMICOLON.slider.headerSchemeChanger(activeSlide, onWinLoad);
				SEMICOLON.slider.subBannerSchemeChanger(activeSlide, onWinLoad);
			}
		...
		subBannerSchemeChanger: function (activeSlide, onWinLoad) {
			if (activeSlide.length > 0) {
				let darkExists = false,
					oldClassesArray,
					noOfOldClasses;
				if (activeSlide.hasClass("dark")) {
					...
					$(".sub-banner-caption").addClass("dark");
				} else {
					if ($body.hasClass("dark")) {
						activeSlide.addClass("not-dark");
					} else {
						$(".sub-banner-caption").removeClass("dark");
					}
				}
			}
		},
	...
	SEMICOLON.documentOnLoad = {
		...
		init: function(){
			...
			SEMICOLON.slider.slickSliderMenu(true);
	...
	var newsSlick = "#slick-news";
	$(newsSlick).parent("div").prepend('<div id="pre-loader"></div>');
	...
	var gallerySlick = "#slick-gallery";
	$(gallerySlick).parent("div").prepend('<div id="pre-loader"></div>');
