        ...
        if(isset($_GET['type'])){
            ...
            if($_GET['type'] == 'searchbox'){
             ...
             elseif($_GET['type'] == 'month'){
                $articles = Article::where('date','like','%'.$_GET['criteria'].'%')->whereStatus('Published');
            }else {
    ...
    public function dates($conditions=null) {
        ...
        if($conditions){
            ...
            foreach($years as $year){
                ...
                $data .= '
                <li>
                    <a href="'.route('news.front.index').'?type=year&criteria='.$year->yr.'"><div>'.$year->yr.'</div></a>
                    <ul>';
                    ...
                    foreach($months as $month){
                        ...
                        $data .= '<li><a class="menu-link" href="'.route('news.front.index').'?type=month&criteria='.$year->yr.'-'.$mo.'"><div>'.date("F", mktime(0, 0, 0, $month->mo, 1)).' ('.$month->total_articles.')</div></a></li>';
                    }
    ...
    public function categories($criteria){
        ...
        $data = '<ul class="mb-0 pb-0">';
                    foreach($categories as $category){
                        ...
                        $data .= '<li style="list-style:none;" class="'.$current.'"><a href="'.route('news.front.index').'?type=category&criteria='.$category->cid.'"><div>'.$category->cat.' ('.$category->total_articles.')</div></a></li>';
                    }
