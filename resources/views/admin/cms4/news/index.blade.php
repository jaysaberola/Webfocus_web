                    ...
                    <div class="table-responsive-lg">
                        <table class="table mg-b-0 table-light table-hover" style="table-layout: fixed;word-wrap: break-word;">
                            ...
                            <tbody>
                                @forelse($news as $new)
                                <?php
                                        $new_name = strlen($new->name) > 60 ? substr($new->name,0,60)."..." : $new->name;
                                        $page_url = strlen(route('news.front.show',$new->slug)) > 50 ? substr(route('news.front.show',$new->slug),0,50)."..." : route('news.front.show',$new->slug);
                                    ?>
                                    <tr id="row{{$new->id}}" class="row_cb">
                                        ...
                                        <td style="overflow: hidden;" title="{{$new->name}}">
                                            <strong @if($new->trashed()) style="text-decoration:line-through;" @endif> {{$new_name}}</strong>
                                            <p class="mg-b-0 tx-gray-500 tx-11">
                                               <a target="_blank" href="{{route('news.front.show',$new->slug)}}">{{$page_url}}</a>
                                            </p>
                                        </td>
                                        <td>
                                            {{$new->category->name}}
