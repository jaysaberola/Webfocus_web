                    ...
                    <div class="d-md-flex bd-highlight">
                        ...
                        <div class="mg-t-10">
                            @if (auth()->user()->has_access_to_route('resource-categories.create'))
                                <a class="btn btn-primary btn-sm mg-b-10" href="{{ route('resource-categories.create') }}">Create a Category</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover" style="table-layout: fixed;word-wrap: break-word;">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 45%;overflow: hidden;">Name</th>
                                        ...
                                        @if($category->trashed())
                                            @if (auth()->user()->has_access_to_route('product.category.restore'))
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('resource-category.restore', $category->id)}}" title="Restore this category"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @endif
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('file-categories.edit'))
                                                    <a class="nav-link" href="{{ route('resource-categories.edit',$category->id) }}" title="Edit Category"><i data-feather="edit"></i></a>
                                                @endif
                                                ...
                                                @if (auth()->user()->has_access_to_route('file.category.change-status'))
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        @if($category->status == 'Active')
                                                            <a class="dropdown-item" href="{{route('resource-category.change-status',[$category->id,'Inactive'])}}" > Inactive</a>
                                                        @else
                                                            <a class="dropdown-item" href="{{route('resource-category.change-status',[$category->id,'Active'])}}"> Active</a>
                                                        @endif
                                                    </div>
                                                @endif
                                            </nav>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('resource-categories.index') }}";
    ...
    <script>
            ...
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    ...
                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('resource-category.multiple.change.status')}}",status,selected_videos);
                    });
                }
                else{
                    post_form("{{route('resource-category.multiple.change.status')}}",status,selected_videos);
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('resource-category.multiple.delete')}}",'',selected_entries);
                });
            }
        }
        ...
        function delete_one_category(id,page){
            ...
            $('#btnDelete').on('click', function() {
                post_form("{{route('resource-category.single.delete')}}",'',id);
            });
        }
