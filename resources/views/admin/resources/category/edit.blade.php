        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Resource Category</li>
                </ol>
            </nav>
            <h4 class="mg-b-0 tx-spacing--1">Edit Resource Category</h4>
        </div>
    </div>

    <div class="row">
        <div class="col-md-6">
            <form action="{{ route('resource-categories.update', $cat->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label class="d-block">Name *</label>
                        <input type="text" name="name" id="name" value="{{ $cat->name }}" class="form-control @error('name') is-invalid @enderror" maxlength="150" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-control custom-switch @error('status') is-invalid @enderror">
                            <input type="checkbox" class="custom-control-input" name="status" id="customSwitch1" {{ ($cat->status == 'Active' ? "checked":"") }}>
                            <label class="custom-control-label" id="label_status" for="customSwitch1">{{ucwords(strtolower($cat->status))}}</label>
                        </div>
                    </div>
