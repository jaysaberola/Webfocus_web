            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-5">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">File Category</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">Manage File Categories</h4>
            </div>
        </div>
            ...
            <div class="col-md-12">
                <div class="filter-buttons">
                    <div class="d-md-flex bd-highlight">
                        ...
                        <div class="mg-t-10">
                            @if (auth()->user()->has_access_to_route('file-categories.create'))
                                <a class="btn btn-primary btn-sm mg-b-10" href="{{ route('file-categories.create') }}">{{__('standard.products.category.create')}}</a>
                            @endif
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table mg-b-0 table-light table-hover" style="table-layout: fixed;word-wrap: break-word;">
                            ...
                            @forelse($categories as $category)
                                <tr id="row{{$category->id}}">
                                    ...
                                    <td>
                                        <strong @if($category->trashed()) style="text-decoration:line-through;" @endif> {{ $category->title }}</strong>
                                    </td>
                                    <td>
                                        @if($category->type == 0)
                                            Department
                                        ...
                                        @if($category->trashed())
                                            @if (auth()->user()->has_access_to_route('product.category.restore'))
                                                <nav class="nav table-options">
                                                    <a class="nav-link" href="{{route('file.category.restore', $category->id)}}" title="Restore this category"><i data-feather="rotate-ccw"></i></a>
                                                </nav>
                                            @endif
                                        @else
                                            <nav class="nav table-options">
                                                @if (auth()->user()->has_access_to_route('file-categories.edit'))
                                                    <a class="nav-link" href="{{ route('file-categories.edit',$category->id) }}" title="Edit Category"><i data-feather="edit"></i></a>
                                                @endif

                                                @if (auth()->user()->has_access_to_route('file-category.single.delete'))
                                                ...
                                                @if (auth()->user()->has_access_to_route('file.category.change-status'))
                                                    <a class="nav-link" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                        <i data-feather="settings"></i>
                                                    </a>
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        @if($category->status == 'Active')
                                                            <a class="dropdown-item" href="{{route('file.category.change-status',[$category->id,'Inactive'])}}" > Inactive</a>
                                                        @else
                                                            <a class="dropdown-item" href="{{route('file.category.change-status',[$category->id,'Active'])}}"> Active</a>
                                                        @endif
                                                    </div>
                                                @endif
                                            </nav>
...
@section('pagejs')
    ...
    <script>
        let listingUrl = "{{ route('file-categories.index') }}";
    ...
    <script>
            ...
            if(parseInt(counter) < 1){
                ...
                if(parseInt(counter)>1){ // ask for confirmation when multiple pages was selected
                    ...
                    $('#btnUpdateStatus').on('click', function() {
                        post_form("{{route('file.category.multiple.change.status')}}",status,selected_videos);
                    });
                }
                else{
                    post_form("{{route('file.category.multiple.change.status')}}",status,selected_videos);
        ...
        function delete_category(){
            ...
            if(parseInt(counter) < 1){
                ...
                $('#btnDeleteMultiple').on('click', function() {
                    post_form("{{route('file.category.multiple.delete')}}",'',selected_entries);
                });
            }
        }
        ...
        function delete_one_category(id,page){
            ...
            $('#btnDelete').on('click', function() {
                post_form("{{route('file-category.single.delete')}}",'',id);
            });
        }
